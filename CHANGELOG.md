...
1. **按钮激活发光效果**
   - 所有按钮、开关、复选框的激活状态添加双层发光效果
   - 发光半径扩大一倍（24px + 40px 双层）
   - 文字也带有发光效果（text-shadow: 16px）
   - 适用于：功能框、模式切换、轨道选择、开关滑块等所有交互元素

2. **发光裁剪问题根本修复**
   - 将所有容器的 `overflow: hidden` 改为 `overflow: visible`
   - 包括：面板内容、面板主体、子菜单等
   - 确保发光效果完全不被裁剪

3. **辉光效果优化**
   - Bloom 半径从 0.5 缩小至 0.2，防止边缘裁剪
   - 相应增加强度补偿，保持视觉效果
   - 最大亮度滑块调整：新值 = 100 + (原值-100) × 0.1
...
2. **独立权重模式**
   ...
   - 权重模式开启：辉光强度 1.5
   - 权重模式关闭：保持默认辉光强度 0.8
   - 修复了关闭权重模式导致辉光完全消失的问题

### 面板边框内侧发光

1. **静态玻璃边框效果**
   - 左侧面板（控制面板、视图面板）：右边、上边、下边发光
   - 右侧面板（数据面板、实验面板）：左边、上边、下边发光
   - 发光朝向场景中心，增强沉浸感

2. **玻璃列表圆角统一**
   - `.glass-list` 圆角与按钮统一为 16px
   - 选中项带有绿色发光边框和文字阴影
