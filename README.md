...
- **轨道形状的三维展示**：s（球形）、p（哑铃形）、d（花瓣形）、f（复杂花瓣形）轨道的真实形态
...
| 4 | s, p, d, f | 16 | 4s, 4px, 4py, 4pz, 4d(xy), 4d(xz), 4d(yz), 4d(x²-y²), 4d(z²), 4f(xyz), 4f(xz²), 4f(yz²), 4f(z³), 4f(z(x²-y²)), 4f(x(x²-3y²)), 4f(y(3x²-y²)) |

共计 **30 种轨道**，涵盖化学教学中常见的所有氢原子轨道。
...
**闪烁类型**（点击 ⚙ 设置）：
- **星空模式**（默认）：每个采样点独立随机闪烁，模拟星空闪烁效果
- **心跳模式**：整个电子云同步明暗变化，模拟真实心跳的双峰节律
- **呼吸模式**：平滑的正弦波明暗变化，柔和的呼吸灯效果
- **扩散模式**：从高密度区域向低密度区域扩散的波纹效果
- **波浪模式**：沿等值面轮廓传播的波浪，展示电子云的三维形状（如P轨道的哑铃形）
...
闪烁模式可以增强演示效果，其中**波浪模式**特别适合展示轨道的等值面形状。
