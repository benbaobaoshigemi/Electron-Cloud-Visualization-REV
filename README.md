...
本项目采用统一的材质配置系统，将所有UI元素的视觉效果参数集中管理在 `materials.json` 配置文件中，确保全局UI材质的一致性和可维护性。

### 三种基础材质

| 材质类型 | 用途 | 特点 |
|----------|------|------|
| **Panel** | 面板背景 | 轻度模糊，突出内容 |
| **Control** | 控件元素 | 统一模糊度，保持交互性 |
| **Dropdown** | 下拉菜单 | 强模糊，确保可读性 |
...
```json
{
  "materials": {
    ...
    "dropdown": {
      "description": "下拉材质 - 保持原有模糊度以确保可读性",
...
材质配置通过 `material-loader.js` 动态加载并应用到CSS变量中：

- **自动加载**：页面加载时自动从 `materials.json` 加载配置
- **动态应用**：将材质参数转换为CSS自定义属性
- **容错处理**：如果配置文件加载失败，使用内置默认值
- **热重载支持**：支持运行时重新加载材质配置
...
- **模式切换栏自动隐藏**：在全屏模式下5秒后自动隐藏
- **鼠标悬停显示**：鼠标移动到屏幕上缘时重新显示

### 自定义材质

要自定义UI材质，请编辑 `materials.json` 文件：

1. 修改相应材质类型的参数值
2. 刷新页面查看效果
3. 参数会自动应用到所有使用该材质的UI元素

**注意**：所有UI元素都严格使用这三种材质中的一种，不允许使用额外的材质参数。
