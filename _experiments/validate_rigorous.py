...
EXCHANGE_COEFFS_BARE = {
    ...
    # Need generic generator for higher L?
}

def get_exchange_scaling(l_target, l_source, k):
    """
    Returns the scaling factor to convert BARE coeff to EXACT AVG coeff.
    Derived from derive_coeffs.py
    """
    # S-orbitals (spherical) always 1.0
    if l_target == 0 or l_source == 0:
        return 1.0
        
    # P-P Intra-shell
    if l_target == 1 and l_source == 1:
        if k == 0: return 1.0
        if k == 2: return 1.2 # 6/5
        
    # D-D Intra-shell
    if l_target == 2 and l_source == 2:
        if k == 0: return 1.0 # F0 always 1.0 to match J cancellation
        if k == 2: return 4.0 # 8/7 / 2/7
        if k == 4: return 40/9 # 4.444...
        
    # P-D Inter-shell
    if (l_target == 1 and l_source == 2) or (l_target == 2 and l_source == 1):
        # All k terms scaled by 2.0
        return 2.0
...
print("RIGOROUS VALIDATION: BARE coeffs * Exact Rational Factors")
print("=" * 70)
...
for symbol, config in CONFIGS.items():
    ...
    for target_name in config.keys():
        ...
        for source_name, (source, R_j, P_j, n_j) in orbitals.items():
            ...
            for k, c_k in coeffs:
                scale = get_exchange_scaling(l_target, l_source, k)
                
                Yk = compute_Yk(P_target, P_j, r, k=k)
                Gk = np.trapezoid(P_target * P_j * Yk, r)
                E_exchange += (n_j / 2) * scale * c_k * Gk
