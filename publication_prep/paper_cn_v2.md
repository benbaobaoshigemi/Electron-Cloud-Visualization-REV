...
本文介绍一个基于量子力学波函数的原子轨道虚拟实验室。对于**氢原子**，本工具使用薛定谔方程的解析解；对于**多电子原子**（He-Kr），使用 **Koga & Thakkar (1999/2000) 发表的多指数 Slater 型基组 (Multi-zeta STO basis sets)**——该基组通过对非相对论 Hartree-Fock 数值波函数进行多指数展开拟合获得，而非简单的 Slater 定则近似。其参数来源于 ab initio 计算，在量子化学领域具有较高的参考价值。**技术创新**包括：支持36种元素；**几何优化杂化算法**（Thomson问题+SVD）支持任意轨道自由组合；实时蒙特卡洛采样与理论径向分布函数的实时对照验证；物理一致的杂化轨道计算（包含交叉项）；跨原子轨道定量比照功能。
