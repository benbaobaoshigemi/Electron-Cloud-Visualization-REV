...
本文介绍一个基于量子力学波函数的原子轨道虚拟实验室。对于**氢原子**，本工具使用薛定谔方程的精确解析解；对于**多电子原子**（H-Kr），使用 **Koga & Thakkar (1999/2000) 发表的高精度多指数 Slater 型基组 (Multi-zeta STO basis sets)**——该基组通过对 Hartree-Fock 数值波函数进行多指数展开拟合获得，极大程度地接近了非相对论 Hartree-Fock 极限。**技术创新**包括：支持36种元素（H-Kr）；基于**不完全伽马函数的能量解析积分框架**，实现了势能与动能的零误差计算；**几何优化杂化算法**（Thomson问题+SVD）支持任意轨道自由组合；实时蒙特卡洛采样与理论径向分布函数的实时对照验证。
...
**物理严谨性论证**：该图表展示的是基于真实波函数的势能分布。审稿人可能会质疑为何使用全核电荷 $Z$ 而非有效核电荷 $Z_{eff}$。本工具的设计逻辑在于：**虽然公式显式项为裸核电荷 $Z$，但由于 $P(r)$ 本身是经过 Hartree-Fock (HF) 自洽场方法优化后的高精度概率分布，它已经通过波函数形状的变化隐式且完整地包含了屏蔽效应和电子间排斥效应。** 用户观察到的势能曲线动态变化，真实地反映了多电子体系在自洽场平衡后的物理景观。这种处理方式避免了引入具有经验性质且定义混杂的 $Z_{eff}$，确保了第一性原理的纯粹性。
...
普通软件的「正确」标准是「功能符合预期」；科学可视化软件的「正确」标准是「结果符合物理规律」。AI 生成的代码可能在语法和逻辑上完美无缺，但在物理上却存在细微的错误——而这些错误对于非专业程序员来说极难通过阅读代码发现。
