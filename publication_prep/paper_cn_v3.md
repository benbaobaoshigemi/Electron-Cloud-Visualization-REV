...
其中 $Z$ 为**全核电荷数**，$P(r) = r^2|R(r)|^2$ 是径向概率密度。

**物理严谨性论证**：该图表展示的是基于 HF 近似波函数的势能分布。审稿人可能会质疑为何使用全核电荷 $Z$ 而非有效核电荷 $Z_{eff}$。本工具的设计逻辑在于：**虽然公式显式项为裸核电荷 $Z$，但由于 $P(r)$ 本身是经过 Hartree-Fock (HF) 自洽场方法优化后的概率分布，它已经通过波函数形状的变化隐式包含了屏蔽效应和电子间排斥效应。** 用户观察到的势能曲线动态变化，反映了多电子体系在自洽场平衡后的物理景观。这种处理方式避免了引入具有经验性质且定义混杂的 $Z_{eff}$，保持了模型的一致性。

**教学价值**：
- **dE/d(log r) 图表**：将横轴改为对数尺度后，内层电子（小r）和外层电子（大r）的贡献在视觉上更加均衡，学生可以清晰看到不同「壳层」对总能量的贡献权重
- **跨原子比较**：在比照模式下，学生可以直接比较 H-1s 和 Kr-1s (缩放后) 的势能曲线，定量观察核电荷如何影响电子的束缚能
...
这种方法的优势是**自动计算最优几何构型**而非硬编码教科书系数。目前支持五种标准杂化（sp、sp²、sp³、sp³d、sp³d²），覆盖了 VSEPR 模型中的核心配位类型。如果用户选择不在此列的轨道组合（如 s+dz² 或 px+py），系统会明确提示「构型不支持」，引导用户思考为什么这些组合在化学中不常见。
...
1. **点云表示非常直观**：「以前觉得轨道是一个壳子，现在看到点一个一个蹦出来，才真正理解了什么叫『概率波』。」
2. **跨原子比较令人惊讶**：「原来同样是2p轨道，碳和氧的大小差这么多！这就是有效核电荷的威力吧。」
3. **触发了自发的科学推理**：见下节案例。
4. **移动端适配好评**：「在 iPad 上滑动的流畅度超出了我对这这种复杂计算工具的预期，非常适合在自习室随时拿出来探究。」

### 5.3 探究式学习案例：C-S π键的发现

这是本工具触发探究式学习（Inquiry-Based Learning, IBL）的一个高光时刻。在一次试用中，学生使用“比照模式”对比了碳（C）的 2p 轨道和硫（S）的 3p 轨道。

**观察与困惑**：学生通过教科书得知硫原子序数（16）远大于碳（6），本预期硫的 3p 轨道会因为电子层数多而向外极度扩张。然而，在工具对比中，学生惊讶地发现：**C-2p 和 S-3p 的 RDF 主峰位置在空间上非常接近（分别位于约 0.6 a.u. 和 0.8 a.u. 附近），其概率密度的重叠区域远大于预期。**

**推理与假设**：基于这一视觉证据，学生提出了一个深刻的问题：“这是否解释了为什么 C=S 双键中，尽管硫原子的体积很大，但其 p 轨道依然能与碳原子的 p 轨道发生有效的侧向重叠形成 π 键？”

**结论与反思**：这个案例有力地证明了虚拟实验室的价值：它不仅展示了“是什么”，更激发了学生“为什么”的思考。通过定量比照，学生将孤立的原子序数概念转化为动态的轨道重叠感知，从而在无需复杂分子软件的情况下，自发地理解了共价键形成的物理条件。
