...
在本项目中，**多选模式**允许用户叠加任意轨道观察干涉效应。例如，选择1s+2s这种「没有化学意义」的叠加——物理引擎照样会基于真实公式渲染结果。用户会看到有趣的干涉条纹，然后产生问题：「为什么化学里不用这种杂化？」这引出了能量、稳定性、对称性匹配的深层讨论。而**杂化模式**严格限于化学上有意义的标准构型（sp、sp²、sp³、sp³d、sp³d²），如果用户选择不支持的组合，系统会明确提示「构型不支持」——这比直接禁用某些轨道更有教学价值。
...
**氢原子（解析解）**：采用薛定谔方程的解析解，波函数表示为拉盖尔多项式与球谐函数的乘积。这是真正的第一性原理——没有任何经验参数或近似。
...
**杂化轨道（几何优化算法）**：采用完整的波函数叠加公式 $P = |\Sigma c_i \psi_i|^2$，**包含交叉项**。杂化系数通过**几何优化算法**动态计算，其核心数学逻辑如下：
1. **方向生成**：利用 **Thomson 问题**（球面点电荷排斥模型），通过梯度下降寻找 $N$ 个杂化轨道的最优空间指向 $v_i$。
2. **基组映射**：构建方向-轨道矩阵 $A_{ij} = \psi_j(\theta_i, \phi_i)$，其中 $\psi_j$ 为参与杂化的原子轨道在指向 $v_i$ 处的角向值。
3. **正交转换**：通过 **SVD 分解** $A = U S V^T$ 求解**正交 Procrustes 问题**，获得最优正交系数矩阵 $Q = U V^T$。
这种算法确保了杂化轨道在物理上的正交性。目前支持五种标准杂化构型：**sp、sp²、sp³、sp³d、sp³d²**——这些覆盖了化学教学中几乎所有常见的配位几何。另外，还支持不同能层的混合选择，这....
...
能量图表采用**冻结轨道近似（Frozen Orbital Approximation）**，直接基于理论径向概率密度 $P(r) = r^2|R(r)|^2$ 计算势能的径向分布：

**势能贡献 $dV/dr$**：
$$\frac{dV}{dr} = \left(-\frac{Z}{r} + V_{ee}(r)\right) \cdot P(r)$$

其中 $Z$ 为核电荷，$V_{ee}(r)$ 为电子间平均排斥势（对于氢原子 $V_{ee}=0$；对于多电子原子，由 Hartree-Fock 自洽场预计算并缓存）。

**势能径向积分 $E(r)$**：
$$E(r) = \int_0^r \frac{dV}{dr'} dr'$$

表示从原点到径向距离 $r$ 处的累积势能贡献。

#### 3.4.2 冻结轨道近似的物理依据

本工具选择冻结轨道近似而非直接使用轨道本征能量 $\varepsilon$，基于以下物理考量：

**"原位能量"与"电离弛豫"的区分**：Hartree-Fock 基组计算得到的轨道本征能量 $\varepsilon$ 反映的是 Koopmans 定理意义下的**电离能近似值**——即假设电离后其他电子不发生弛豫时的能量变化。然而，这一数值本质上混合了"电子被移除"这一破坏性观测过程的影响。

本工具的设计目标是展示电子**"原位"（in-situ）**处于原子中时的能量分布景观，而非电离过程所涉及的能量。因此，势能可视化直接计算电子与核及其他电子之间的静电相互作用，不引入本征能量。这使得图表展示的是**电子在量子态中真实经历的势能环境**，而非经过电离过程平均化后的派生量。

**物理一致性**：由于 $P(r)$ 本身是经过 Hartree-Fock 自洽场方法优化后的概率分布，它已经通过波函数形状的变化隐式包含了屏蔽效应和电子间排斥效应。用户观察到的势能曲线，反映了多电子体系在自洽场平衡后的物理景观。

#### 3.4.3 算法设计考量

能量可视化采用理论概率密度 $P(r)$ 而非采样统计直方图 $f(r)$，这一设计决策基于以下物理和数值考量：

1. **核区奇点问题**：库仑势 $V_{nuc} = -Z/r$ 在 $r \to 0$ 处趋于负无穷。若使用采样直方图 $f(r)$，微小的统计涨落 $\delta f$ 在核附近会被放大为 $\sim Z \cdot \delta f / r$ 的巨大误差，导致曲线在视觉上剧烈抖动。

2. **物理意义的一致性**：对于给定的量子态，其能量分布是确定的物理属性，不应随采样过程波动。采用理论分布 $P(r)$ 展示的是该量子态所蕴含的能量景观，而非观测过程的统计涨落。

3. **与RDF验证的区分**：径向概率分布图表（Section 3.3）保留采样直方图与理论曲线的对照，以演示大数定律和蒙特卡洛方法的收敛性；能量图表则直接展示理论结果，二者在教学功能上形成互补。

#### 3.4.4 教学价值

- **定量化学直觉**：学生可直接观察不同轨道对总势能的贡献权重，理解为何内层电子贡献更大的束缚能
- **跨原子比较**：比照模式下可定量对比不同原子同一轨道的势能分布，观察有效核电荷如何影响电子的束缚程度
- **物理模型的边界意识**：通过明确区分"原位能量"与"电离能"，帮助学生建立对理论模型适用范围的正确认知
...
本工具采用**几何优化算法**动态计算杂化系数，其核心步骤如下：

**步骤一：方向生成**

系统首先确定 $N$ 个杂化轨道的空间指向。对于标准杂化构型，这些方向需满足特定的几何约束：
- **sp**：线性，两端对称 (180°)
- **sp²**：平面三角形 (120°)
- **sp³**：正四面体 (109.5°)
- **sp³d**：三角双锥
...
方向生成采用**Thomson问题**的梯度下降求解：将 $N$ 个点视为球面上的同性电荷，通过最小化库仑排斥能获得最均匀的分布。对于受约束的杂化（如sp²仅在xy平面内），算法自动识别参与轨道的角向空间，并将优化限制在相应的子空间内。

**步骤二：系数矩阵求解**

构建**方向-轨道矩阵** $A_{ij} = Y_j(\theta_i, \phi_i)$，其中 $Y_j$ 为第 $j$ 个参与轨道在第 $i$ 个杂化方向处的球谐函数值。

通过**奇异值分解 (SVD)** $A = U \Sigma V^T$ 求解正交Procrustes问题，获得最优正交系数矩阵 $Q = UV^T$。此方法确保杂化轨道之间保持正交性，符合量子力学对波函数的基本要求。
...
本工具附带一份**探索指南**文档 (`DISCOVER_GUAID.md`)，提供了一系列引导式探究路径。该文档遵循"虚拟实验室"的设计哲学：不给出标准答案，而是提出开放性问题，激发使用者的自主探索和科学推理。
...
该文档在说明中明确指出：工具展示的是**单电子轨道能量**，基于Hartree-Fock近似；涉及电子相关效应的现象（如过渡金属的异常电子排布）超出了单电子图景的描述范围。这种透明的方法论陈述有助于学生建立对理论模型适用边界的正确认知。
...
本项目选择在「原子轨道」处划定边界：这条线以上是「波函数层级的答案」，这条线以下是「人类构造的模型」。这样的边界保持了核心承诺的一致性。
...
1. **技术创新**：支持H-Kr范围内STO轨道的教学工具；**几何优化杂化算法**（Thomson问题+SVD正交Procrustes）支持五种标准杂化（sp/sp²/sp³/sp³d/sp³d²）；物理一致的杂化轨道（含交叉项）；跨原子比照功能；实时采样与理论曲线验证；Web Worker异步计算确保流畅交互。
