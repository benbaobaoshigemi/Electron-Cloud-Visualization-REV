                ...
                if (colors && positions) {
                    ...
                    if (shouldRecompute && Hydrogen && Hydrogen.density3D_real && Hydrogen.orbitalParamsFromKey) {
                        ...
                        if (isHybridMode && Hydrogen.sortOrbitalsForHybridization) {
                            state.waveOrbitalParams = Hydrogen.sortOrbitalsForHybridization(state.waveOrbitalParams);
                        }
                        ...
                        if (state.waveOrbitalParams.length > 0) {
                            ...
                            // 【关键修改】杂化模式也使用每点独立轨道计算
                            const useHybridPerPoint = isHybridMode && state.pointOrbitalIndices && state.hybridOrbitalPointsMap;
                            ...
                            for (let i = 0; i < processCount; i++) {
                                ...
                                // 根据模式选择轨道参数和密度计算方式
                                let density = 0;
                                if (useHybridPerPoint && coeffMatrix) {
                                    // 【杂化模式】每个点使用其所属的杂化轨道计算密度
                            ...
                            if (usePerPointOrbital || useHybridPerPoint) {
                                // 比照模式/杂化模式：对每个轨道的点分别排序
                                // 这样每个轨道的等值面是独立计算的
                    ...
                    for (let i = 0; i < processCount; i++) {
                        ...
                        if (isHybridMode && state.hybridOrbitalVisibility) {
                            const hybridIdx = state.pointOrbitalIndices ? state.pointOrbitalIndices[i] : 0;
                            if (state.hybridOrbitalVisibility[hybridIdx] === false) {
                                // 轨道被隐藏，保持全黑
                                colorArray[i3] = 0;
                                colorArray[i3 + 1] = 0;
                                colorArray[i3 + 2] = 0;
...
window.ElectronCloud.Scene.onSamplingCompleted = function () {
    ...
    // 【新增】杂化模式下，启用杂化轨道按钮
    if (state.isHybridMode && window.ElectronCloud.UI.enableHybridOrbitalButtons) {
        window.ElectronCloud.UI.enableHybridOrbitalButtons();
    }
...
window.ElectronCloud.Scene.clearAngularOverlay = function () {
    ...
    // 重置相关的UI开关状态
    const angular3dToggle = document.getElementById('angular-3d-toggle');
    if (angular3dToggle && angular3dToggle.checked) {
        angular3dToggle.checked = false;
    }
};
